...
class VrptwGraph:
    def __init__(self, file_path, rho=0.1):
        super()
        # node_num Nombre de nœuds
        # node_dist_mat La distance entre les nœuds (matrice)
        # pheromone_mat La densité d'informations sur le chemin entre les nœuds
        self.node_num, self.nodes, self.node_dist_mat, self.vehicle_num, self.vehicle_capacity \
            = self.create_from_file(file_path)
       # taux de volatilisation des phéromones rho
        self.rho = rho
        # Créer une matrice de phéromones
        ...
        # Matrice d'informations heuristiques
    ...
    def create_from_file(self, file_path):
        # Lire l'emplacement du point de service et du client à partir du fichier
        node_list = []
    ...
    def _cal_nearest_next_index(self, index_to_visit, current_index, current_load, current_time):
        """
       Trouvez le next_index accessible le plus proche
