...
class Ant:
    ...
    def move_to_next_index(self, next_index):
        ...
        if self.graph.nodes[next_index].is_depot:
            # Si l'emplacement suivant est un dépôt, effacez la charge du véhicule, etc.
            self.vehicle_load = 0
            self.vehicle_travel_time = 0

        else:
            # Mettre à jour la charge du véhicule, la distance de conduite, le temps
            self.vehicle_load += self.graph.nodes[next_index].demand
            # S'il est antérieur à la fenêtre de temps requise par le client (ready_time), vous devez attendre
    ...
    def check_condition(self, next_index) -> bool:
        """
        Vérifier si le passage au point suivant satisfait les contraintes
        :param next_index:
        :return:
        """
        ...
        # Vérifiez si vous pouvez retourner au magasin de service après avoir rendu visite à un certain passager
        ...
        # Ne peut pas servir les passagers en dehors des heures prévues
    ...
    def cal_next_index_meet_constrains(self):
        """
        Trouver tous les clients accessibles depuis l'emplacement actuel (ant.current_index)
        :return:
        """
    ...
    def cal_nearest_next_index(self, next_index_list):
        """
        Sélectionnez parmi les clients à sélectionner, le client le plus proche de l'emplacement actuel (ant.current_index)
    ...
    def try_insert_on_path(self, node_id, stop_event: Event):
        """
        Essayez d'insérer node_id dans travel_path actuel
        La position insérée ne peut pas violer les restrictions de charge, de temps et de distance de déplacement
        S'il y a plusieurs emplacements, trouvez le meilleur emplacement
        :param node_id:
        :return:
        """
        ...
        for insert_index in range(len(self.travel_path)):
            ...
            # Trouver le dépôt le plus proche devant insert_index
           ...
           # check_ant part de front_depot_index
            check_ant = Ant(self.graph, self.travel_path[front_depot_index])

            # Laissez check_ant passer par le point où l'indice dans le chemin commence de front_depot_index à insert_index-1
            for i in range(front_depot_index+1, insert_index):
                check_ant.move_to_next_index(self.travel_path[i])

            
            # Commencez à essayer de visiter les nœuds dans l'index_to_visit trié
            ...
            # Si le node_id peut être atteint, assurez-vous que le véhicule peut retourner au dépôt
            for next_ind in self.travel_path[insert_index:]:
                ...
                # S'il n'est pas possible de retourner au dépôt, revenir au niveau précédent
                else:
                    break
    ...
    def insertion_procedure(self, stop_even: Event):
        """
        Essayez de trouver un emplacement approprié pour chaque nœud non visité et insérez-le dans le travel_path actuel
        La position insérée ne peut pas violer les restrictions de charge, de temps et de distance de déplacement
        :return:
        """
        ...
        # Jusqu'à ce qu'aucun des nœuds non visités ne puisse être inséré avec succès
        while success_to_insert:

            success_to_insert = False
            
            # Obtenez des nœuds non visités
            ind_to_visit = np.array(copy.deepcopy(self.index_to_visit))

           # Obtenez la demande de clients en visite, triée par ordre décroissant
    ...
    def local_search_once(graph: VrptwGraph, travel_path: list, travel_distance: float, i_start, stop_event: Event):
        ...
        # Trouver l'emplacement de tous les dépôts dans le chemin
        depot_ind = []
        for ind in range(len(travel_path)):
            if graph.nodes[travel_path[ind]].is_depot:
                depot_ind.append(ind)

        # Divisez self.travel_path en plusieurs segments, chaque segment commence par le dépôt et se termine par le dépôt, appelé itinéraire
                    ...
                    for end_a in range(start_a, min(depot_ind[i], start_a + 6)):
                        for start_b in range(depot_ind[j - 1] + 1, depot_ind[j]):
                            for end_b in range(start_b, min(depot_ind[j], start_b + 6)):
                                ...
                                # Déterminer si la route modifiée a est faisable
                                ...
                                # Déterminer si la route modifiée b est faisable
                                ...
                                if success_route_a and success_route_b:
                                    ...
                                    if new_path_distance < travel_distance:
                                        ...
                                        # Supprimer un des dépôts reliés entre eux dans le chemin
    ...
    def local_search_procedure(self, stop_event: Event):
        """
        Utilisez cross pour effectuer une recherche locale sur le travel_path actuel qui a visité tous les nœuds du graphique
        :return:
        """
